...
<body>
    ...
    <thead>
        <tr>
            ...
            <th>
                <span class="flex items-center">Adres</span>
            </th>
            <th>
    ...
    <tbody>
        ...
        foreach (getRestaurantOrders($_GET['edit_restaurant']) as $row) {
            ...
            echo '<option value="pending" ' . ($row['order_status'] == 'pending' ? 'selected' : '') . '>Beklemede</option>';
            echo '<option value="in_progress" ' . ($row['order_status'] == 'in_progress' ? 'selected' : '') . '>Hazırlanıyor</option>';
            echo '<option value="completed" ' . ($row['order_status'] == 'completed' ? 'selected' : '') . '>Tamamlandı</option>';
            echo '<option value="canceled" ' . ($row['order_status'] == 'canceled' ? 'selected' : '') . '>İptal Edildi</option>';
            echo '</select>';
            echo '<button type="submit" name="order_status_update" class="btn btn-success mt-2">Kaydet</button>';
            echo '</form>';
            echo '</td>';
            ...
            echo '<input type="hidden" name="order_id" value="' . $row['order_id'] . '">';
            echo '<input type="hidden" name="restaurant_id" value="' . $_GET['edit_restaurant'] . '">';
            echo '<button type="submit" name="delete_order" class="btn btn-danger mt-2">Sil</button>';
            echo '</div>';
            echo '</td>';
