...
if (isset($_POST["login"])) {
    ...
    $conditions = ['username' => $l_usrname, 'password' => $password];
    $sonuc = select('users', $conditions);

    if ($sonuc) { // Eğer kullanıcı bulunduysa (sorgu sonucu boş değilse)
...
if (isset($_POST["order_status_update"])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: restaurant_paneli.php?edit_restaurant=" . $_POST['restaurant_id']);
        exit;
    } else {
